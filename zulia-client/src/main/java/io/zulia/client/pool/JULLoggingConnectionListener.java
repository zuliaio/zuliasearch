...
public class JULLoggingConnectionListener implements ConnectionListener {
	private static final Logger LOG = Logger.getLogger(JULLoggingConnectionListener.class.getName());
	...
	public void exceptionClosing(ZuliaConnection zuliaConnection, Exception e) {
		ZuliaBase.Node node = zuliaConnection.getNode();
		LOG.log(Level.SEVERE, "Exception closing connection to " + node.getServerAddress() + ":" + node.getServicePort(), e);
	}
